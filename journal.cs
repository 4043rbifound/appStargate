...
namespace appStargate
{
    public partial class journal : Form
    {
        ...
        private Point _pointDepart; // Pour le déplacement du formulaire
        ...
        public journal(string nomPlanete, int numeroMission)
        {
            ...
            // ─── CHARGEMENT DES DONNÉES DEPUIS LA BASE (Sécurisé) ───

            // Journal de bord (avec tri temporel de la V2)
            using (SQLiteCommand cmd = new SQLiteCommand(
                "SELECT dateJ, commentaires FROM JournalDeBord WHERE nomPlanete = @nom AND numero = @num " +
                "ORDER BY CASE WHEN dateJ LIKE '__/__/____' " +
                "THEN substr(dateJ,7,4)||substr(dateJ,4,2)||substr(dateJ,1,2) " +
                "ELSE replace(dateJ,'-','') END ASC", Connexion.Connec))
            {
            ...
            // Stockage des infos de la mission dans les variables
            ...
            // ─── BILAN DES CAPTURES ───────────────────────────────────────
            string nomTableBilan = $"BilanCapture{nomPlanete}-{numeroMission}";
            ...
            // Remplissage des grilles de données
            ...
            // Calculs et affichages initiaux
            CalculerTotaux();
            MettreAJourCompteur();
        }
        ...
        {
            MettreAJourCompteur();
            // Re-formate proprement la date en français après navigation
            if (DateTime.TryParse(lblDateJournal.Text, out DateTime parsedDate))
            {
                lblDateJournal.Text = parsedDate.ToString("dd/MM/yyyy");
            }
        }
        ...
        private void AfficherNotif(string message)
        {
            ...
            notif.BackColor = Color.FromArgb(39, 174, 96); // Vert succès
        ...
        // ─── ACTIONS SUR LES FENÊTRES / NAVIGATION GÉRALES ─────────────────
        private void pictureBox10_Click(object sender, EventArgs e) { this.Close(); }
        private void pctQuitter_Click(object sender, EventArgs e) { Application.Exit(); }
        private void pictureBox19_Click(object sender, EventArgs e) { this.WindowState = FormWindowState.Minimized; }
        ...
        // ─── DÉPLACEMENT DU FORMULAIRE SANS BORDURE VIA SOURIS ───────────
        private void pictureBox8_MouseDown(object sender, MouseEventArgs e) { _pointDepart = e.Location; }
        ...
        private void pictureBox9_MouseDown(object sender, MouseEventArgs e) { _pointDepart = e.Location; }
        ...
        // ─── GÉNÉRATION DU PDF VIA ITEXTSHARP ─────────────────────────────
        private void btnEditerPdf_Click(object sender, EventArgs e)
        {
            
        }

        private void btnEditerPdf_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // Bilan des captures (Tableau PDF complexe)
                doc.Add(new Paragraph("Bilan des captures :", fontSousTitre));
                doc.Add(new Paragraph("\n"));

                string nomTableBilanPdf = $"BilanCapture{_nomPlanete}-{_numeroMission}";
                DataTable dtBilan = _ds.Tables.Contains(nomTableBilanPdf) ? _ds.Tables[nomTableBilanPdf] : null;
                ...
                // Fermeture et ouverture automatique du PDF généré
                doc.Close();
            ...
            {
                MessageBox.Show($"Erreur lors de l'édition du PDF : {ex.Message}", "Erreur");
            }
        }
    }
}